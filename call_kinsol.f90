...
! fkpset: Se especifica el jacobiano a utilizar. fdata es la f de fkfun. 
    ...
    integer(KIND=1), intent(inout) :: ier ! Kinsol error flag !! Integer of 8 bits between -128 and 127
    ...
    integer(KIND=1) :: msbpre ! max. numb. of nonlinear iterations without a call to the preconditioner setup function. integer of 8 bits [-128,127]
    integer :: i
...
! rout - real outputs
    ...
    call fkinsetiin('MAX_SETUPS', msbpre, ier)  ! maximum number of iterations without prec. setup 
    call fkinsetiin('MAX_NITER', maxiter, ier)  ! maximum number of Nonlinear iterations (Default=200)
...
! -2.0 then u(i) will be constrained to be u(i) < 0.0
    do i = 1, neq  !constraint vector
        constr(i) = 0.0  ! Indicate that no inequality constraints should be imposed on the solution vector.
    enddo
...
! Esta funcion indica a Kinsol que debe buscar las funciones fkpsol y fkpset (precondiciones dadas por el usuario)
...
! U = x1 is an array containing the initial guess on input, and the solution on return. 
...
!           2 to indicate apparent stalling (small step), or a negative value to indicate an error or failure.
    if (ier < 0) then
