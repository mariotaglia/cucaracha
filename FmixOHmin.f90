...
!    use FreeEnergy, only: checknumber
    ...
    fmixOHmin=0
    do iR = 1, dimR
        fmixOHmin = fmixOHmin + xOHmin(iR)*(dlog(xOHmin(iR))-1.0 -dlog(expmuOHmin))*(dfloat(iR)-0.5)*delta/Radio
        fmixOHmin = fmixOHmin - xOHminbulk*(dlog(xOHminbulk)-1.0 -dlog(expmuOHmin))*(dfloat(iR)-0.5)*delta/Radio
    enddo
